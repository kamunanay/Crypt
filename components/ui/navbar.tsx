...
export default function Navbar() {
  ...
  return (
    <nav className={cn(
      ...
      scrolled ? 'bg-[#0b0d1a]/85 backdrop-blur-xl border-b border-white/5' : 'bg-[#0b0d1a]/40 backdrop-blur-lg'
    )}>
          ...
          return (
            <li key={item.href}>
              <Link href={item.href} className={cn(
                ...
                {item.name}
                {isActive && <span className="absolute bottom-0 left-0 w-full h-[2px] bg-gradient-to-r from-[#f5c842] to-[#d4a020] rounded-full" />}
              </Link>
            </li>
          );
        })}
