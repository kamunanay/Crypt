...
export async function getForexRates(): Promise<ForexData> {
  if (!API_KEY || API_KEY === '') return getFallbackData();
  try {
    const response = await axios.get(`https://api.currencyfreaks.com/v2.0/rates/latest?apikey=${API_KEY}`);
    if (response.status !== 200 || !response.data?.rates) return getFallbackData();
